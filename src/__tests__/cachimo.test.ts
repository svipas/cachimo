	...
	describe("put", () => {
		it("should throw error if key type is invalid", () => {
			...
			invalidTypes.forEach((key) =>
				expect(() => cachimo.put(key as any, "")).toThrowError()
			);
		});
		...
		it("should throw error if timeout type is invalid", () => {
			...
			invalidTypes.forEach((timeout) =>
				expect(() => cachimo.put("key", "value", timeout as any)).toThrowError()
			);
		});

		it("should throw error if callback type is invalid", () => {
			const invalidTypes = [NaN, {}, [], false, true, "", " ", 1, -1, 0];
			invalidTypes.forEach((callback) =>
				expect(() =>
					cachimo.put("key", "value", 1, callback as any)
				).toThrowError()
			);
		});
